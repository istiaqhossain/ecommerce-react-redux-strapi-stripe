...
export default function CartPanel({ cartPanelToggle, cartPanelToggleHandler }) {
  ...
  let productList = products.map((product) => (
    <li key={product.id} className="flex py-6">
      <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
        <img
          src={import.meta.env.VITE_UPLOAD_URL + product.image}
  ...
  if (products.length > 0) {
          ...
          <div className="mt-6 flex justify-center text-center text-sm text-gray-500">
            <p>
              <button
                ...
                type="button"
                className="font-medium text-text-600 hover:text-text-500"
              >
                Reset Cart
              </button>
            </p>
    ...
    cartContent = (
      <>
        <div className="mt-6 flex flex-col gap-3 justify-center text-center text-sm text-gray-500">
          <p className="text-gray-900">Cart is empty.</p>
          <p>
            <button
              onClick={cartPanelToggleHandler}
              type="button"
              className="font-medium text-gray-600 hover:text-gray-500"
            >
              Continue shopping.
  ...
  return (
    <>
      <div
        className={cartPanelToggle ? "relative z-20" : "relative z-20 hidden"}
